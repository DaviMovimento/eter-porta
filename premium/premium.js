...
function evento(nome, dados = {}) {
  /* o sensor do mapa de calor ouve todos os degraus do funil */
  try { window.__calor?.(nome, { origem: dados.origem, edicao: EDICAO?.n }); } catch (e) {}
...
async function iniciar() {
  DADOS = await (await fetch(new URL('../edicoes.json?v=202609241140', ONDE_MORO))).json();
...
function montarChegada() {
  ...
  if (caixaPasse && !caixaPasse.querySelector('.mock-passe')) {
    ...
    f.innerHTML = `<img src="${CASA}mockup-assinatura.webp?v=202609241140"
  ...
  /* tudo montado: a chegada pode aparecer (ver html.montando no molde) */
  requestAnimationFrame(() => document.documentElement.classList.remove('montando'));
}
...
function montarFundo() {
  ...
  const parede = `${BASE}edicoes/${EDICAO.n}/${paredeArq}?v=202609241140`;
...
function montarEspiada() {
  ...
  const carregar = n => new Promise(ok => {
    ...
    const im = new Image();
    /* esperar também a DECODIFICAÇÃO: o onload chega antes de a imagem
       estar pintável, e era esse vão que piscava na virada */
    im.onload = () => im.decode ? im.decode().then(() => ok(pag(n, 800)), () => ok(pag(n, 800))) : ok(pag(n, 800));
    im.onerror = () => ok(pag(n, 800));
    im.src = pag(n, 800);
  });

  function por(slot, n) {
    /* já está nesta página: não mexe, senão o srcset cai e volta e a
       folha pisca à toa */
    if (existe(n) && +slot.dataset.n === n && slot.getAttribute('src')) return;
    if (existe(n)) {
      /* A ORDEM É O QUE MATA A PISCADA. O srcset fazia o navegador
         escolher o @1400 — que a virada nunca aqueceu — e jogar fora o
         @800 pronto, deixando a folha vazia no meio do giro. Agora a
         folha ASSENTA no @800 já aquecido, e o @1400 sobe por trás:
         quando termina de decodificar, o srcset entra e a troca é
         invisível, porque o arquivo já está no cache. */
      slot.removeAttribute('srcset');
      slot.src = pag(n, 800);
      slot.style.visibility = 'visible';
      slot.dataset.n = n;
      const hd = new Image();
      hd.onload = () => (hd.decode ? hd.decode() : Promise.resolve()).then(() => {
        if (+slot.dataset.n !== n) return;   /* a folha já virou de novo */
  ...
  function legendar(origem) {
    ...
    /* o rótulo de páginas saiu do DOM; escrever nele às cegas derrubava a
       página inteira no primeiro null */
    const pn = $('#pulso-n');
    if (pn) {
      pn.textContent = folha === 0 ? 'Capa'
  ...
  async function irParaFolha(k, origem) {
    ...
    if (frente) {
      ...
      const abrindo = folha === 0;
      folha = k;
      if (abrindo) spread.classList.add('abrindo');   /* a esquerda ainda é a mesa */
    ...
    /* A PÁGINA DE BAIXO TROCA NO MEIO DO GIRO, não no fim. Aos 60% a
       folha já passou dos 90 graus e cobre o lado onde vai pousar: é a
       hora de pôr ali a página nova, que assim já está pintada quando a
       folha deita. Trocar no fim deixava um quadro de papel em branco
       entre a folha sumir e a imagem aparecer — a piscada. */
    setTimeout(() => {
      if (frente) { por(slotEsq, e1); spread.classList.remove('abrindo'); }
      else por(slotDir, d1);
    }, semMovimento() ? 0 : 570);
    setTimeout(() => {
      if (frente) { por(slotEsq, e1); spread.classList.remove('abrindo'); } else por(slotDir, d1);
...
function blocoFim() {
  ...
  d.innerHTML = `
    ...
    <section class="passe">
      <figure class="mock-passe"><img src="${CASA}mockup-assinatura.webp?v=202609241140"
...
function mostrarPorteiro(v) {
  ...
  /* "Voltar" mentia: quem bate nesta parede estava abrindo OUTRA edicao,
     nunca esteve na do mes nesta sessao, e clicava esperando retomar a
     leitura de onde saiu — caía numa edição que não tinha aberto. O botao
     leva para a edicao que ela TEM direito de ler, entao e isso que ele diz. */
  $('#mes-voltar').textContent = v.edicaoEmCurso ? `Ler a ED${v.edicaoEmCurso}` : 'Voltar';
...
function ligar() {
  ...
  document.body.addEventListener('click', e => {
    ...
    if (!b) return;
    /* botão colado num card que JÁ lista os entregáveis não repete a
       oferta em popup: vai direto (cadastrado) ou ao formulário curto */
    const aoLadoDosItens = ['capa', 'fim'].includes(b.dataset.passe);
    aoLadoDosItens ? irParaCheckout(b.dataset.passe) : abrirOferta(b.dataset.passe);
  });
}
